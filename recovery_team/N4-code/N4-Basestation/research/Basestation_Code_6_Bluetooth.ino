/**
 * N4 Base Station Code 6 - Bluetooth Edition
 * 
 * PRODUCTION BASE STATION with Bluetooth Serial Output
 ...
 * - Baud rate: 115200
 * 
 * This code listens for REAL beacons from the rocket and forwards
 * telemetry to both USB and Bluetooth for maximum compatibility.
 */
...
void sendTelemetryJSON() {
  ...
  doc["wifi_rssi"] = beacon_rssi;  // Use measured beacon RSSI
    ...
    if (payload[i] == 0xDD) {
      ...
      if (parseCSV(csv_data, telemetry)) {
        ...
        if (lastOperationMode != telemetry.operation_mode) {
          ...
          sendLogMessage("INFO", ("Rocket mode: " + String(mode)).c_str(), "BaseStation");
...
void processCommand(String command) {
  ...
  if (command == "ARM" || command == "DISARM" || command == "RESET" ||
    ...
    sendLogMessage("INFO", "Mode: MQTT, BEACON, AUTO_ON, AUTO_OFF, STATUS", "BaseStation");
    sendLogMessage("INFO", "PWM: SET_PWM:{json}, PWM_STATUS", "BaseStation");
  }
  else if (command != "") {
    sendLogMessage("WARNING", ("Unknown: " + command).c_str(), "BaseStation");
  }
}
...
void setup() {
  ...
  sendLogMessage("INFO", "N4 Base Station Code 6 - Bluetooth Edition (460800 baud)", "BaseStation");
  sendLogMessage("INFO", "Production beacon receiver with Bluetooth output", "BaseStation");
  ...
  sendLogMessage("INFO", "Mode: MQTT, BEACON, AUTO_ON, AUTO_OFF, STATUS", "BaseStation");
  sendLogMessage("INFO", "PWM: SET_PWM:{json}, PWM_STATUS", "BaseStation");
  sendLogMessage("INFO", "Waiting for rocket beacons...", "BaseStation");
}
