/**
 * N4 Base Station Code 8 - XBee + Mega UART Bridge Edition
 * 
 * PRODUCTION BASE STATION with XBee and Mega UART bridge output
 * - Receives real beacon telemetry from rocket (29-field CSV)
 * - Receives XBee UART telemetry (29-field CSV, 900MHz radio)
 * - Outputs via USB Serial AND UART2 to Mega
 ...
 * - Mega UART bridge (GPIO 16(8)/17(9)), UART2)
 * - Baud rate: USB=115200, XBee=115200, Mega UART=115200
 * 
 * UART Assignments:
 * - UART0 (Serial): USB debugging/telemetry
 * - UART1 (XBeeSerial): XBee Pro 900HP
 * - UART2 (MegaSerial): Mega UART bridge @ 115200 baud
 */
...
// ====== Mega UART Bridge Configuration (UART2) ======
// Bluetooth module removed: UART2 is now dedicated to Mega bridge.
HardwareSerial MegaSerial(2);
#define Mega_TX 17  // ESP32 TX2 -> Mega RX
#define Mega_RX 16  // ESP32 RX2 <- Mega TX
...
const char* DEVICE_ID = "ESP32:N4_BASE_Mega_1";
...
sample from xctu receiver

//651,0,0,0.09,0.98,0.02,5.13,88.49,-9.27,-2.99,-3.02,0.00000000,0.00000000,0.00,186460,847.93,26.94,0.71,0.00,0,0,0,0,13.59,3.58,0,0,13.04,39.58
...
void sendLogMessage(const char* level, const char* message, const char* source) {
  ...
  // USB Serial only — do NOT send to MegaSerial.
  // Sending logs to Mega caused a feedback loop: parse-fail log -> MegaSerial TX
  // blocks -> XBee RX buffer fills -> more parse-fails -> more logs -> never recovers.
  Serial.print("LOG:");
...
void sendHeartbeat() {
  ...
  // USB Serial only — Mega has its own heartbeat for its own COM port.
  // Sending heartbeat JSON to MegaSerial caused PARSE_FAIL spam on the Mega.
...
void sendTelemetryJSON() {
  ...
  // USB Serial: full JSON for Python server / laptop
  Serial.print(jsonString);

  // MegaSerial: ONLY the compact 29-field CSV prefixed with "CSV:"
  // Mega parses this CSV and builds its own JSON for its USB Serial output.
  // Do NOT send the large JSON or LOG messages to MegaSerial — at 115200 baud
  // a 900-byte write blocks for ~80ms, starving XBee reads and the keypad.
  // Build CSV into a larger buffer and send exact byte length to avoid
  // truncation/concatenation on the AVR receiver.
  char csvFrame[512];
  int n = snprintf(csvFrame, sizeof(csvFrame),
           "CSV:%lu,%u,%u,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.6f,%.6f,%.2f,%lu,%.2f,%.2f,%.2f,%.2f,%u,%u,%u,%u,%.2f,%.2f,%u,%d,%.2f,%.2f\n",
           ...
           telemetry.power_rail_low, telemetry.wifi_rssi,
           telemetry.kalman_altitude, telemetry.kalman_vertical_velocity);

  // Determine actual bytes to send (handle truncation and snprintf errors)
  int sendLen = 0;
  if (n < 0) {
    // formatting error, skip sending
    sendLogMessage("ERROR", "Failed to format CSV frame", "BaseStation");
    sendLen = 0;
  } else if (n >= (int)sizeof(csvFrame)) {
    // truncated; ensure last byte is newline
    sendLen = (int)sizeof(csvFrame) - 1;
    csvFrame[sendLen - 1] = '\n';
    csvFrame[sendLen] = '\0';
    sendLogMessage("WARNING", "CSV truncated before newline", "BaseStation");
  } else {
    sendLen = n;
    // ensure newline at end
    if (sendLen > 0 && csvFrame[sendLen - 1] != '\n') {
      if (sendLen < (int)sizeof(csvFrame) - 1) {
        csvFrame[sendLen++] = '\n';
        csvFrame[sendLen] = '\0';
      } else {
        csvFrame[sendLen - 1] = '\n';
  ...
  if (sendLen > 0) {
    MegaSerial.write((const uint8_t*)csvFrame, sendLen);
  }
}
...
// Route command to the active channel (ESP-NOW / XBee / MQTT stub)
void sendCommandViaActiveChannel(const String& cmd) {
  if (currentMode == MODE_XBEE) {
    // Send over XBee (UART1)
    XBeeSerial.println(cmd);
    sendLogMessage("INFO", ("[XBEE TX] Command sent: " + cmd).c_str(), "BaseStation");
  }
  else if (currentMode == MODE_BEACON) {
    // Queue for ESP-NOW send via existing mechanism
    lastCommand = cmd;
    commandPending = true;
    commandSentTime = millis();
    sendLogMessage("INFO", ("[ESP-NOW] Command queued: " + cmd).c_str(), "BaseStation");
  }
  else if (currentMode == MODE_MQTT) {
    // Not implemented: forwarding via MQTT from base station
    sendLogMessage("WARNING", "MQTT command forwarding not implemented in base station", "BaseStation");
  }
  else {
    // AUTO: default to ESP-NOW queueing
    lastCommand = cmd;
    commandPending = true;
    commandSentTime = millis();
    sendLogMessage("INFO", ("[AUTO->ESP-NOW] Command queued: " + cmd).c_str(), "BaseStation");
  }
}
...
void handleSerialCommands() {
  ...
  if (MegaSerial.available()) {
    String command = MegaSerial.readStringUntil('\n');
    command.trim();
    // Strip CMD: prefix — Mega queues commands as "CMD:ARM\n" etc.
...
void processCommand(String command) {
  if (command.startsWith("SET_PWM:") || command.startsWith("set_pwm:")) {
    ...
    sendCommandViaActiveChannel(fullCommand);
    char msg[250];
  ...
  // Simple immediate commands — route over active channel
  if (command == "ARM" || command == "DISARM" || command == "RESET" ||
      command == "MAIN_ON" || command == "MAIN_OFF" ||
      command == "DROGUE_ON" || command == "DROGUE_OFF" ||
      command == "POP_TEST_DROGUE" || command == "POP_TEST_MAIN") {
    sendCommandViaActiveChannel(command);
    sendLogMessage("INFO", ("Command: " + command).c_str(), "BaseStation");
  }
  else if (command == "CMD_MQTT_MODE" || command == "MQTT_MODE" || command == "MQTT") {
    sendCommandViaActiveChannel("CMD_MQTT_MODE");
    // Keep local mode in sync
    currentMode = MODE_MQTT;
    sendLogMessage("INFO", "Local mode set to MQTT", "BaseStation");
  }
  else if (command == "CMD_BEACON_MODE" || command == "BEACON_MODE" || command == "BEACON") {
    sendCommandViaActiveChannel("CMD_BEACON_MODE");
    currentMode = MODE_BEACON;
    sendLogMessage("INFO", "Local mode set to Beacon (ESP-NOW)", "BaseStation");
  }
  else if (command == "CMD_XBEE_MODE" || command == "XBEE_MODE" || command == "XBEE") {
    sendCommandViaActiveChannel("CMD_XBEE_MODE");
    currentMode = MODE_XBEE;
    sendLogMessage("INFO", "Local mode set to XBee", "BaseStation");
  }
  else if (command == "CMD_AUTO_FALLBACK_ON" || command == "AUTO_FALLBACK_ON" || command == "AUTO_ON") {
    sendCommandViaActiveChannel("CMD_AUTO_FALLBACK_ON");
  }
  else if (command == "CMD_AUTO_FALLBACK_OFF" || command == "AUTO_FALLBACK_OFF" || command == "AUTO_OFF") {
    sendCommandViaActiveChannel("CMD_AUTO_FALLBACK_OFF");
  }
...
void sendCommandToRocket() {
  ...
  // Only send queued commands via ESP-NOW when we're in Beacon mode
  // or AUTO mode (default to ESP-NOW). If another channel was active,
  // the command will have already been sent by sendCommandViaActiveChannel.
  if (currentMode != MODE_BEACON && currentMode != MODE_AUTO) {
    commandPending = false;
    return;
  }

  if (millis() - commandSentTime > COMMAND_TIMEOUT) {
    sendLogMessage("WARNING", ("ESP-NOW command timeout: " + lastCommand).c_str(), "BaseStation");
    commandPending = false;
    return;
  }
...
void setup() {
  Serial.begin(115200);
  MegaSerial.begin(Mega_BAUD, SERIAL_8N1, Mega_RX, Mega_TX);
  delay(1000);
  MegaSerial.begin(115200, SERIAL_8N1, Mega_RX, Mega_TX);
  
  // test serial communication 
  MegaSerial.println("HELLO_MEGA");
  Serial.println("[ESP] Test string sent to Mega");



  sendLogMessage("INFO", "========================================", "BaseStation");
  sendLogMessage("INFO", "N4 Base Station - XBee + Mega Bridge Edition", "BaseStation");
  sendLogMessage("INFO", "========================================", "BaseStation");
  sendLogMessage("INFO", "Device ID: ESP32:N4_BASE_Mega_1", "BaseStation");
  sendLogMessage("INFO", "XBee: UART1, RX=34, TX=32, RSSI=35, 115200 baud", "BaseStation");
  sendLogMessage("INFO",  "Mega bridge: UART2, RX=16, TX=17, 115200 baud", "BaseStation");
  ...
  sendLogMessage("INFO",  "Mega UART bridge enabled @ 115200 baud", "BaseStation");
  ...
  for (int i = 0; i < 3; i++) {
    ...
    Serial.print(jsonString);
    // No MegaSerial here — Mega sends its own startup heartbeats
...
void loop() {
  ...
  if (millis() - lastStatusHeartbeat > 10000) {
    ...
    // USB Serial only — status JSON is for Python server on this port only.
    // Do NOT send to MegaSerial: Mega cannot parse it, and it overflows the
    // AVR's 64-byte UART RX buffer causing corrupt concatenated messages.
    Serial.print("STATUS:");
